  ...
  return `
    <div class="plan-usage-dots">
      ${Array.from({ length: limit })
        .map(
          (_, i) =>
            `<span class="plan-usage-dot ${i < current ? "is-filled" : ""}"></span>`
        )
        .join("")}
    </div>
  `;
...
function openLeadModal(lead = null) {
  ...
  openModal({
        ...
        <div class="modal-grid-2">
          ...
          <div class="field">
            ...
            <select id="lead-status" name="status">
              ...
              <option value="Cotización enviada" ${lead?.status === "Cotización enviada" ? "selected" : ""}>Cotización enviada</option>
    ...
    actions: `
      <button class="btn btn-secondary" id="cancel-lead-modal" type="button">
        Cancelar
      </button>
  ...
  bindModalFormSubmit("lead-modal-form", async (form) => {
    ...
    try {
      if (!data.company?.trim() && !data.name?.trim()) {
        showToast("Agrega al menos la empresa o el contacto");
        return;
      }
    ...
    .map(
      (lead) => `
        <tr>
          ...
          <td>
            <div class="btn-row">
              <button class="btn btn-secondary btn-sm js-edit-lead" data-id="${lead.id}">
                Editar
              </button>

              <button class="btn btn-secondary btn-sm js-quote-lead" data-id="${lead.id}">
                Cotizar
              </button>

              <button
                class="btn btn-secondary btn-sm js-delete-lead ${canDelete ? "" : "btn-disabled"}"
                data-id="${lead.id}"
                ${canDelete ? "" : "disabled"}
              >
                Eliminar ${canDelete ? "" : '<span class="badge-pro">PRO</span>'}
              </button>
            </div>
          </td>
        </tr>
...
function bindLeadActions() {
  ...
  document.querySelectorAll(".js-delete-lead").forEach((btn) => {
    btn.onclick = async () => {
      ...
      if (!confirm(`Eliminar prospecto "${lead.name || "Sin nombre"}"?`)) return;
      ...
      try {
        ...
        console.error(error);
        showToast("No se pudo eliminar el prospecto");
      }
    };
  });
}
...
async function loadLeads() {
  try {
    ...
    console.error(error);
    showToast("No se pudieron cargar los prospectos");
  }
}
      ...
      <div class="app-view-grid">
        ...
        <article class="app-panel">
          <div class="form-grid-2 mb-4">
            <div class="field">
              <label>Buscar</label>
              <input id="leads-search" placeholder="Nombre, empresa, email..." />
            </div>
